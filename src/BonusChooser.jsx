    ...
    const updateAttacker = () => {
      setAttacker(a => {
        ...
        if (bonus.type === SKILL_BONUS) {
          temp.skillBonus = parseFloat(temp.skillBonus) + normalizeAmount(bonus.amount)
        } else if (bonus.type === ATK_BONUS) {
          temp.atkBonus = parseFloat(temp.atkBonus) + normalizeAmount(bonus.amount)
        } else {
          temp.hp = parseFloat(temp.hp) + normalizeAmount(bonus.amount) * temp.worth;
        }
        return temp
      })
    }
...
function getBonusValue(attacker, bonus) {
  ...
  let dmgChange;
  if (bonus.type === SKILL_BONUS) {
    dmgChange = (1 + attacker.atkBonus) * attacker.critChance * amount
  } else if (bonus.type === ATK_BONUS) {
    dmgChange = (1 + (attacker.critMultiplier + attacker.skillBonus) * attacker.critChance) * amount
  } else {
    return 0;
  }
